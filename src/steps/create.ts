...
export class PgNestedMutationCreateStep<
  TNestedResource extends PgTableResource = PgTableResource,
  TParentResource extends PgTableResource = PgTableResource,
> extends ExecutableStep {
  ...
  private leftTable: PgTableResource;
  ...
  private parentAttributes: {
    name: keyof GetPgResourceAttributes<TParentResource>;
    depId: number;
    pgCodec: PgCodec;
  }[] = [];

  private attributes: {
    name: keyof GetPgResourceAttributes<TNestedResource>;
    depId: number;
    pgCodec: PgCodec;
  }[] = [];
  ...
  constructor(
    ...
    parent: PgNestedAttributeMap<TParentResource>,
    args: PgNestedAttributeMap<TNestedResource>,
  ) {
    super();
    this.leftTable = rel.leftTable;
    ...
    Object.entries(parent).forEach(([key, value]) => {
      ...
      const attribute = Object.entries(this.leftTable.codec.attributes).find(
        ([k, _v]) => k === key,
      );
      if (!attribute) {
        throw new Error(
...
>(
  ...
  return new PgNestedMutationCreateStep<TNestedResource, TParentResource>(
    rel,
    parent,
    args,
  );
